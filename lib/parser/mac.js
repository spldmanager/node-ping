...
MacParser.prototype._processFooter = function (line) {
    if(line.indexOf('% packet loss')>1){
        this._response.ploss = line.split('% packet loss')[0].split('received, ')[1];
        console.log(this._response.ploss)
        if(this._response.ploss.indexOf('errors')>1){
            this._response.ploss = this._response.ploss.split('errors, ')[1];
        }
    }    
